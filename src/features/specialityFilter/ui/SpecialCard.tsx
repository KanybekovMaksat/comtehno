  ...
  return (
    ...
    >
      <Stack direction={"row"} spacing={1} className="mb-4 items-center">
        ...
        <Typography className="text-[#0F766E] font-normal" variant="caption">
          {category?.name}
        </Typography>
      </Stack>
      <Box className="mb-20 relative">
        <Typography
          className="relative z-10 text-xl font-medium uppercase"
          variant="h3"
        >
          {title}
        </Typography>
        <Typography className="relative z-10 text-tundora" variant="caption">
          {specialty}
        </Typography>
        <img
      ...
      <div className="r-sm:hidden relative z-10 w-full text-center hover:bg-[#096059] py-3 bg-[#0D9488] text-white rounded-xl normal-case">
        Поступить
      </div>
    </Link>
  );
