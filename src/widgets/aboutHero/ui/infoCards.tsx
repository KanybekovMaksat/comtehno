...
const AnimatedCounter: React.FC<{ targetValue: number }> = ({
  targetValue,
  isCount,
}) => {
...
export const AboutPageCards = () => {
  ...
  const cards = [
    ...
    { value: 1000, isCount: true, label: t("aboutPage.cards.students") },
    { value: 4000, isCount: true, label: t("aboutPage.cards.graduates") },
    { value: 8, isCount: true, label: t("aboutPage.cards.mentors") },
  ];
    ...
    <div className="flex gap-4 justify-center items-center r-md:flex-col r-sm:items-center">
      {cards.map((item, index) => (
        ...
        >
          ...
          >
            <AnimatedCounter targetValue={item.value} isCount={item.isCount} />
          </Typography>
          <Typography
            variant="body2"
